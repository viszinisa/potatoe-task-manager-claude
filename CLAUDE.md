...
- Do not exit this project's directory (`/home/artis/work2/potatoe-task-manager`). You do not need my permission to edit project files.
...
- Commit after each phase/task completes, not in one lump at the end. Uncommitted work is unrecoverable if something later deletes or overwrites it, and a large mixed tree cannot be split cleanly afterwards (interleaved edits to one file by several phases can only be committed together).
    ...
    - **Finishing work means committing AND pushing BOTH repos** (see "Two repos" below), not just the outer one. Check `git status` in `/home/artis/work2/potatoe-task-manager` and in `/home/artis/work2/potatoe-task-manager/.claude` before declaring done. Both repos track `origin/main` and take a plain `git push`.

## Two repos

- **Outer** `/home/artis/work2/potatoe-task-manager` → `git@github.com:viszinisa/potatoe-task-manager.git` — the stack itself (compose, `_docker/`, `api/`, `frontend/`, README).
- **Nested** `/home/artis/work2/potatoe-task-manager/.claude` → `git@github.com:viszinisa/potatoe-task-manager-claude.git` — agent config only: skills, hooks, bin, agents, and `CLAUDE.md` (the repo-root `CLAUDE.md` is a symlink to `.claude/CLAUDE.md`, so the `.claude` repo owns it).
...
- Every stack-affecting change (compose.yml, anything under `_docker/`, api or frontend behaviour, nginx/DB config) gets one short line in `_docs/changelog.md` in the same turn — newest first, ISO date, one line per entry. This obligation re-arms on EVERY commit that changes stack behaviour, including review fix rounds: a first-cut line does not cover a later fix round's behaviour change. Docs, tests and process-record commits carry no entry.
- Sub-agents never end on a signal-only or bare-status message ("✅ done"). The harness surfaces a sub-agent's last message as its report, so that last message must carry the full report content. A sub-agent return consisting only of a status line or an emoji is a **failed dispatch**: the orchestrator does not guess at the work — re-request the report via `SendMessage` (the agent's transcript persists) or re-dispatch, and treat the work as unverified until a real report exists.
