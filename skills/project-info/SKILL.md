...
- **`App\Service\TaskSeqAllocator::allocate()` must run before any other
  ...
  needed in-memory.
- **E2E per-worker identities are `parallelIndex`-keyed, not test-keyed** — `frontend/e2e/helpers/users.ts::allocateUser` maps `role + parallelIndex` to seeded logins (`admin${idx+1}` etc.); `POOL_SIZE` (10) must stay ≤ compose's `LDAP_USERS_PER_ROLE` **and** ≤ the `NAMES` table in `_docker/ldap/bootstrap/generate-data.sh` (10 per role; bootstrap hard-fails `no name entry for '<sam>'` past it) — move all three in lockstep, never just one.
